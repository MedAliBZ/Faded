...
void Diplay_Xo(Xo* xo,SDL_Surface* ecran)
{
    for(int i=1;i<4;i++)
    {
        ...
        {
            if(xo->tab[i][j]==1){
               SDL_BlitSurface(xo->xoS,&xo->posxo[1],ecran,&xo->xoR[i][j]); 
            }
            else if (xo->tab[i][j]==2){
                SDL_BlitSurface(xo->xoS,&xo->posxo[2],ecran,&xo->xoR[i][j]);
            }   
        }
    }
    SDL_Flip(ecran);
...
bool Play_Xo(SDL_Surface *ecran)
{
    Xo xo;
    Init_Xo(&xo);
    SDL_Surface *back=IMG_Load("img/table.png");
    SDL_Event event;
    int gameover=0;
    while(gameover==0)
    {
